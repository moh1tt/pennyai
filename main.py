...
if __name__ == "__main__":
    fetch_reddit_posts(
        f"data/reddit_posts_{today_str}.parquet",
        limit_per_sub=10, subreddit_list=subreddits_to_fetch)

    preprocess(f"data/reddit_posts_{today_str}.parquet",
               f"data/processed_reddit_posts_{today_str}.parquet")

    enrich_tickers_with_yfinance(
        f"data/processed_reddit_posts_{today_str}.parquet", f"data/processed_yfinance_info_{today_str}.parquet")

    merge_reddit_yfinance(
        f"data/processed_reddit_posts_{today_str}.parquet",
        f"data/processed_yfinance_info_{today_str}.parquet",
        f"data/llm_ready_dataset_{today_str}.parquet"
    )

    upload_to_db("data/duckdb/pennyai.duckdb",
                 f"data/llm_ready_dataset_{today_str}.parquet",
                 "training")
